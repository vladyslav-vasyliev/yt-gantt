        ...
        <Stack spacing={3} sx={{ pt: 2 }}>
          ...
          <Stack direction="row" spacing={2} useFlexGap sx={{ alignItems: "flex-start", flexWrap: "wrap" }}>
            <TextField
              ...
              error={!!sizeErr} helperText={sizeErr ?? "(issue, activities) ⇒ число дней или null (размер не задан — план не строится)"}
              slotProps={{
                input: {
                  endAdornment: (
      ...
      {tab === "info" && (
        <Stack>
          <Typography variant="body1" sx={{ marginTop: 2 }}>
            ...
            Resolved (у родителя — до его собственного завершения; если дети идут дальше,
            до самого позднего ребёнка тянется слабо окрашенный пунктирный хвост).
            Размер задаёт плановый бар, факт рисуется параллельно. Родитель отсчитывается
            ...
            стандартной иерархии это <code>parent for</code>). Если поле размера пустое или
            неизвестно, лямбда возвращает null: у родителя план строится по границам
            поддерева, у листа плановый бар не рисуется.
          </Typography>
